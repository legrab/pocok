...
public sealed class ReadinessConcurrencyTests
{
    ...
    public async Task CancellingManyCallersDoesNotCancelSharedReadiness()
    {
        ...
        Task[] cancelled = [.. Enumerable.Range(0, 64).Select(_ => source.WaitUntilReadyAsync(cycle, cancellation.Token))];
        Task[] survivors = [.. Enumerable.Range(0, 64).Select(_ => source.WaitUntilReadyAsync(cycle, cancellation.Token))];
    ...
    public async Task FailurePublishesAtomicSnapshotBeforeWaitersResume()
    {
        ...
        Task<ReadinessSnapshot>[] snapshots = [.. Enumerable.Range(0, 64)
