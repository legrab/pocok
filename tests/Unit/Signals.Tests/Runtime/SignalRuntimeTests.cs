...
public sealed class SignalRuntimeTests
{
    ...
    {
        var source = new TestSignalSource();
        await using SignalRuntime runtime = RuntimeFor(source, new SignalRuntimeOptions(2));
        await using SignalConnection<int> connection = (await runtime.ConnectAsync<int>(Address)).Value!;
        await TestAsync.UntilAsync(() => source.ActiveSubscriptions == 1);

        for (var value = 1; value <= 5; value++) source.Emit(value);
    ...
    public async Task ConfirmedWriteConvertsAndPublishesEvidence()
    {
        ...
        SignalResult<SignalWriteResult<int>> written =
            await connection.WriteAsync(42, SignalWriteConsistency.ReadAfterWrite);
    ...
    {
        var source =
            new TestSignalSource(capabilities: SignalSourceCapabilities.Read | SignalSourceCapabilities.Subscribe)
            {
    ...
    public async Task WriteRequiresBothCapabilityAndWriterInterface()
    {
        ...
        SignalResult<SignalWriteResult<int>> result =
            await connection.WriteAsync(1, SignalWriteConsistency.Acknowledged);
    ...
    private sealed class FailingThenStreamingSource : ISignalSubscriber
    {
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            await foreach (SignalSample<object?> sample in _samples.Reader.ReadAllAsync(cancellationToken)
                               .ConfigureAwait(false)) yield return sample;
        }
...
internal sealed class ManualTimeProvider(DateTimeOffset initialTime) : TimeProvider
{
    ...
    private sealed class ManualTimer : ITimer
    {
        ...
        internal ManualTimer(ManualTimeProvider provider, TimerCallback callback, object? state, TimeSpan dueTime,
            TimeSpan period)
        {
            _provider = provider;
            _callback = callback;
