...
public sealed class FileStringLocalizerCultureTests
{
    ...
    public async Task ExactCultureFallsBackPerKeyToParentThenInvariant()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.json"), """{"InvariantOnly":"invariant","Shared":"invariant"}""");
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.de.json"), """{"ParentOnly":"parent","Shared":"parent"}""");
    ...
    {
        using var directory = TemporaryDirectory.Create();
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.json"), """{"Shared":"json","JsonOnly":"json-only"}""");
    ...
    public async Task ReversingFormatPrecedenceReversesSameCultureConflict()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.resx"), FileLocalizationTestData.CreateResx(("Shared", "resx")));
    ...
    public async Task ExactCultureResxBeatsInvariantJson()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.de.resx"), FileLocalizationTestData.CreateResx(("Shared", "german-resx")));
        using var culture = new TemporaryCulture("de-AT");
    ...
    {
        using var directory = TemporaryDirectory.Create();
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.json"), """{"Z":"invariant-z","A":"invariant-a","Shared":"invariant"}""");
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "Messages.de.json"), """{"B":"parent-b","Shared":"parent"}""");
        ...
        public LocalizedString this[string name, params object[] arguments]
        {
            get
            {
                ...
                return result.ResourceNotFound
                    ? result
                    : new LocalizedString(name, string.Format(System.Globalization.CultureInfo.InvariantCulture, result.Value, arguments), false);
            }
        }
