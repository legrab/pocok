...
public sealed class ReleaseReadinessCatalogTests
{
    ...
    public void AllNonRetiredLibrariesAreAlphaReleasableAndHaveTagTriggers()
    {
        ...
        using var catalog = JsonDocument.Parse(File.ReadAllText(Path.Combine(root, "eng", "packages.json")));
    ...
    {
        var root = RepositoryRoot.Path;
        using var packages = JsonDocument.Parse(File.ReadAllText(Path.Combine(root, "eng", "packages.json")));
        var showcaseCatalogPath = Path.Combine(
            root,
            "showcase",
            "src",
            "Pocok.Showcase.Web",
        ...
        using var showcase = JsonDocument.Parse(File.ReadAllText(showcaseCatalogPath));

        var expected = packages.RootElement.GetProperty("packages").EnumerateArray()
            .Where(package => package.GetProperty("state").GetString() != "Retired")
            .Select(package => package.GetProperty("id").GetString()!)
            .Order(StringComparer.Ordinal)
            .ToArray();
        var actual = showcase.RootElement.GetProperty("packages").EnumerateArray()
            .Select(package => package.GetProperty("id").GetString()!)
            .Order(StringComparer.Ordinal)
            .ToArray();
