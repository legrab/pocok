...
The Licensing editor builds `LicenseDocument` and `LicenseValidationContext` values in memory and calls the public licensing APIs. On explicit generation it creates an ephemeral signing key pair, returns only the signed envelope and public key, and verifies the result with `LicenseReader`; the private key remains server-side in memory and is never displayed, logged, persisted, or returned. The editor does not accept license files or perform filesystem access. Password fields are masked in the browser and secrets are omitted from result previews and diagnostics.
