...
PACKAGE_SLUGS = (
    ...
    "app-defaults-logging-serilog",
    "modularity-contracts",
    "modularity",
    "app-defaults-modularity",
    "background-work",
    "scripting",
...
def read_text(
    ...
    with opener.open(url, timeout=3) as response:
        return response.status, html.unescape(
    ...
    with (
        ...
        try:
            endpoints = [
                ...
                "/health/ready",
                "/",
                *(f"/packages/{slug}" for slug in PACKAGE_SLUGS),
                "/system",
            ]
            ...
            if any(
                "@bind:get=" in page or "@bind:set=" in page
                for page in sample_pages
            ):
                raise RuntimeError(
                    "A native sample binding was emitted as an inert HTML attribute."
                )
            if control_value(conversion, "source-value") != "300":
                raise RuntimeError(
                    "Conversion default sample did not populate the source editor."
                )
            if control_value(licensing, "license-id") != "demo-license":
                raise RuntimeError(
                    "Licensing default sample did not populate the license ID."
                )
            if control_value(licensing, "licensed-modules") != "Reporting, Export":
                raise RuntimeError(
                    "Licensing default sample did not populate the module editor."
                )
            if control_value(scripting, "timeout") != "1000":
                raise RuntimeError(
                    "Scripting default sample did not populate execution options."
                )
            ...
            if (
                status != 200
                or 'lang="hu"' not in hungarian_home
                or "Főoldal" not in hungarian_home
            ):
                raise RuntimeError(
                    "Hungarian culture cookie did not update shell rendering."
                )

            _, hungarian_conversion = read_text(
                opener,
                f"{base_url}/packages/conversion",
            )
            if "Konverzió futtatása" not in hungarian_conversion:
                raise RuntimeError(
                    "Hungarian culture cookie did not update Conversion rendering."
                )

            _, hungarian_scripting = read_text(
                opener,
                f"{base_url}/packages/scripting",
            )
            if "Szkript futtatása" not in hungarian_scripting:
                raise RuntimeError(
                    "Hungarian culture cookie did not update Scripting rendering."
                )

            _, hungarian_licensing = read_text(
                opener,
                f"{base_url}/packages/licensing",
            )
            if "Licenc ellenőrzése" not in hungarian_licensing:
                raise RuntimeError(
                    "Hungarian culture cookie did not update Licensing rendering."
                )

            print(
                "Smoke test passed:",
                ", ".join(endpoints),
                "and EN/HU cookie localization",
            )
            return 0
        finally:
            process.terminate()
