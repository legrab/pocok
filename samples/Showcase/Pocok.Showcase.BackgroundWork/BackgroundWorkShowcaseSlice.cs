...
public sealed class BackgroundWorkShowcaseSlice : ShowcaseSlice<BackgroundWorkInput, BackgroundWorkOutput>, IShowcasePackageCoverage
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        string code = BackgroundWorkRecipeRenderer.Render(input);
...
public static class BackgroundWorkRecipeRenderer
{
    ...
    {
        int interval = Math.Clamp(input.Capacity, 1, 10_000);
        string failureOptions = input.IncludeOptional
            ? """
                ...
                """
            : string.Empty;
        string explicitStop = input.IncludeLifecycle
            ? "\nawait runner.StopAsync(cancellationToken);"
            : string.Empty;
        string timeoutFault = input.IncludeOptional
            ? "\n        .OnFault<TimeoutException>(exception => " +
              "logger.LogWarning(exception, \"Background work timed out.\"))"
            : string.Empty;
        string observationOutcome = input.IncludeLifecycle
        ...
        string recipe = input.Mode switch
        {
            "coalesce" => $$"""
    ...
    internal static async Task CompileProofAsync(Task backgroundTask, CancellationToken cancellationToken)
    {
        ...
        await TaskRepeater.RepeatAsync(_ => ValueTask.CompletedTask,
            new TaskRepeaterOptions { Interval = TimeSpan.FromMilliseconds(1), MaximumIterations = 1 }, cancellationToken);
    }
}
