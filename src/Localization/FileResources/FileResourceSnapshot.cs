...
internal sealed class FileResourceSnapshot
{
    ...
    {
        foreach (var cultureName in EnumerateCultureNames(culture, true))
        ...
        bool includeParents)
    {
        ...
        {
            if (!Resources.TryGetValue(cultureName, out FrozenDictionary<string, string>? entries)) continue;
    ...
    private static IEnumerable<string> EnumerateCultureNames(CultureInfo culture, bool includeParents)
    {
        ...
        while (true)
        {
            ...
            if (current.Equals(CultureInfo.InvariantCulture)) yield break;
