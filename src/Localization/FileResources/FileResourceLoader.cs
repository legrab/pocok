...
internal static class FileResourceLoader
{
        ...
        FileResourceSnapshot? previous)
    {
            ...
            settings.MissingFileBehavior == MissingLocalizationFileBehavior.RetainLastKnownGood)
        {
            ...
            if (missingPath is not null)
                throw new IOException(
                    $"Localization source '{missingPath}' disappeared; the last-known-good snapshot was retained.");
        }

        var byCultureAndFormat =
            new Dictionary<(string Culture, LocalizationFileFormat Format), FrozenDictionary<string, string>>();
                 ...
                 sources.GroupBy(source => source.CultureName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            {
                if (!byCultureAndFormat.TryGetValue((cultureGroup.Key, format),
                        out FrozenDictionary<string, string>? entries)) continue;

                foreach (KeyValuePair<string, string> entry in entries) merged.TryAdd(entry.Key, entry.Value);
            }
    ...
    {
        if (!string.Equals(Path.GetDirectoryName(Path.GetFullPath(path)), settings.ContainingDirectory,
    ...
    {
        if (!Directory.Exists(settings.ContainingDirectory)) return [];
