    ...
    internal static FrozenDictionary<string, string> Parse(string path)
    {
        try
        {
            ...
            foreach (XElement data in root.Elements().Where(element => element.Name.LocalName == "data"))
            {
                ...
                var hasUnsupportedChildren =
                    children.Any(element => element.Name.LocalName is not "value" and not "comment");
                if (values.Length != 1 || values[0].Elements().Any() || hasUnsupportedChildren)
                    throw new FormatException(
                        $"RESX resource '{nameAttribute.Value}' must contain exactly one plain-text value element and an optional comment.");
            ...
            return entries.ToFrozenDictionary(StringComparer.Ordinal);
                                              ...
                                              or FormatException)
        {
            throw new FormatException($"Failed to parse RESX localization file '{path}'. {exception.Message}",
                exception);
        }
    }
}
