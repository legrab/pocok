...
internal static class JsonLocalizationParser
{
        ...
        bool allowTrailingCommas)
    {
        try
        {
            ...
            exception is IOException or UnauthorizedAccessException or DecoderFallbackException or JsonException
                or FormatException)
        {
            throw new FormatException($"Failed to parse JSON localization file '{path}'. {exception.Message}",
                exception);
        }
    }
    ...
    {
        return prefix is null ? segment : $"{prefix}.{segment}";
    }
}
