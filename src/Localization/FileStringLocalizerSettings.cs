    ...
    Action<FileLocalizationStatus>? StatusChanged)
{
    internal static FileStringLocalizerSettings Create(FileStringLocalizerOptions options)
    {
        ...
        {
            throw new ArgumentException("BaseName must be a root-relative path without alternate stream syntax.", nameof(options));
        }
        ...
        {
            throw new DirectoryNotFoundException($"The watched localization directory does not exist: {containingDirectory}");
        }
