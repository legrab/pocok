...
public sealed class FileStringLocalizer : IAsyncDisposable, IStringLocalizer
{
    ...
    public LocalizedString this[string name, params object[] arguments]
    {
        get
        {
            ...
            if (value.ResourceNotFound || arguments.Length == 0)
            {
                return value;
            }
        ...
        try
        {
            ...
            for (var attempt = 0; attempt <= _settings.ReloadRetryCount; attempt++)
            {
                ...
                catch (Exception exception) when (IsReloadFailure(exception))
                {
                    ...
                    if (attempt == _settings.ReloadRetryCount)
                    {
                        break;
                    }
                }
    ...
    {
        if (FileResourceLoader.MayBeCandidatePath(_settings, eventArgs.FullPath))
        {
    ...
    private void RequestWatcherReload()
    {
        ...
        if (runner is null || Volatile.Read(ref _disposed) != 0)
        {
            return;
        }
    ...
    private async Task DisposeCoreAsync()
    {
        ...
        if (_reloadRunner is not null)
        {
            await _reloadRunner.DisposeAsync().ConfigureAwait(false);
        }
