...
public sealed class DebouncedTaskRunner : IAsyncDisposable
{
    ...
    public Task RequestAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (completionToStart is not null) _ = DrainAsync(completionToStart);
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (cancelLifetime) await _lifetime.CancelAsync();
    ...
    private async Task DrainAsync(TaskCompletionSource completion)
    {
        try
        {
            while (true)
            {
                if (!await WaitForQuietPeriodAsync(completion).ConfigureAwait(false)) return;
    ...
    private void CompleteDrainAsSucceeded(TaskCompletionSource completion)
    {
        lock (_gate)
        {
            if (!ReferenceEquals(_drainCompletion, completion)) return;
