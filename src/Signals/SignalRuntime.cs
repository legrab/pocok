        ...
        while (true)
        {
            ...
            lock (_lifecycleSync)
            {
                ...
                creation = _entries.GetOrAdd(
                    address,
                    static (key, runtime) => new EntryCreation(
                        () => Task.Run(() => runtime.CreateEntryAsync(key))),
                    this);
            ...
            {
                if (creation.ExitAndIsLast())
                {
                    _ = CleanupUnusedCreationAsync(address, creation);
                }
            ...
            lock (_lifecycleSync)
            {
                ...
                if (!disposed)
                {
                    created.Value!.TryAcquire(out connection);
                }
            }
            ...
            {
                return SignalResult.Success(connection);
            }
