...
internal sealed class SharedSignalSubscription
{
    ...
    internal void AddSubscriber(SignalSubscriberBuffer subscriber)
    {
        lock (_sync)
        {
            ...
            {
                subscriber.Enqueue(_lastSample);
            }
        }
    }
    ...
    private void ScheduleStalenessLocked(SignalQuality quality)
    {
        ...
        if (quality != SignalQuality.Good || Options.StaleAfter is not { } staleAfter)
        {
            return;
        }
    ...
    private void MarkStale(long generation)
    {
        lock (_sync)
        {
            if (_stopping || generation != _staleGeneration || _lastSample is not { Quality: SignalQuality.Good } last)
            {
                return;
            }
