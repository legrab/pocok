...
public static partial class ScriptImportParser
{
    ...
    public static IReadOnlyList<ScriptReference> FindImports(string? content)
    {
        ...
        return
        [
            .. ImportRegex().Matches(content).Select(static match =>
                new ScriptReference(match.Groups["Name"].Value, match.Groups["Module"].Value))
        ];
    }

    internal static string RemoveImports(string content)
    {
        return ImportRegex().Replace(content, string.Empty);
    }

    internal static string ImportedComment(ScriptReference reference, int depth)
    {
        return $"// {new string(' ', depth * 2)}#imported {reference.Name} from {reference.Module}";
    }

    [GeneratedRegex(
        @"^[ \t]*//[ \t]*#import[ \t]*(?<Name>[A-Za-z0-9_]+)[ \t]+from[ \t]+(?<Module>[A-Za-z0-9_]+)[ \t]*(;[^\r\n]*)?$",
