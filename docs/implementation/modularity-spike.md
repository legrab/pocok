...
**MVP owner:** `docs/plans/repository-finalization.md` R4
...
This is trusted in-process extension loading, not a sandbox. MVP does not add scanning of arbitrary DLLs, intermediate service providers, unload/hot reload, remote acquisition, child containers, shadow copy, or module-to-module dependency orchestration.

McMaster.NETCore.Plugins is not an MVP dependency. Reconsider it only through a new post-MVP design if concrete requirements exceed the current internal load-context class; implementation agents must not research or substitute it during R4.

## Existing functionality to reuse

- `AddPocokModules(...)` and `ModuleLoadOptions` for host registration/configuration;
- `IModuleCatalog`/`ModuleDescriptor`/`ModuleDiagnostic` for status and safe diagnostics;
...
Pass only when fixtures prove private managed dependency loading, shared contract identity, optional/required failures, duplicate IDs, malformed manifests, platform/architecture filtering, deterministic order, multiple `IEnumerable<TContract>` implementations, and diagnostics without path/secret leakage on both operating systems. Package and Showcase staging must use the same manifest/load behavior.

On a loader failure, preserve the failing fixture and diagnostic. Fix the current BCL implementation while retaining the chosen public boundary. Do not fall back to default-context scanning, load every DLL, swallow `ReflectionTypeLoadException`, build an intermediate provider, or change publication eligibility until all commands pass.

After the gate passes, keep API maturity `Experimental` and set `publicationPolicy` to `PrereleaseOnly`; stable publication remains a separate future compatibility decision.
