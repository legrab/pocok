...
# 13. One-shot implementation contract for the coding agent

## 13.1 Mission

Transform the current `pocok.zip` repository into the target state described here in one working session, using organized commits. Preserve the useful behavior of the current implementation, remove unjustified package boundaries, repair release engineering, implement AppDefaults and Modularity, and leave a buildable, testable, packable public reference repository.

## 13.2 Inputs

- Current repository: `pocok.zip`.
- Legacy evidence: `origin.zip`, read-only and proprietary.
...
# 14. Ordered implementation and commit plan

The agent should create small coherent commits in this order. It may combine adjacent commits only when separation would leave the repository uncompilable, but must retain project-level organization.
